...
final class IdGenerationContextSpec extends AnyWordSpec with Matchers with GivenWhenThen {
  private val _in_phase52_spec =
    afterWord("in spec:entity-collection-identity, example:id-generation, rules:R1,R4, phase:52")
  private val _purpose =
    Gen.nonEmptyListOf(Gen.alphaNumChar).map(_.mkString)

  "IdGenerationContext" must _in_phase52_spec {
    ...
    "keep opaque IDs in an independent purpose sequence" in {
      ...
      val left = IdGenerationContext.deterministic(IdGenerationContext.DEFAULT_NAMESPACE, clock, "opaque-seed")
      val right = IdGenerationContext.deterministic(IdGenerationContext.DEFAULT_NAMESPACE, clock, "opaque-seed")
    ...
    "preserve the runtime namespace while the EntityId carries its collection independently" in {
      ...
      val context = IdGenerationContext.deterministic(IdGenerationContext.DEFAULT_NAMESPACE, clock, "collection-seed")

      When("the ordinary Entity ID capability creates an Entity ID")
      val id = context.entityId(collection, "blob.register")

      Then("the opaque ID remains self-describing when parsed")
      id.major shouldBe IdGenerationContext.DEFAULT_NAMESPACE.major
      id.minor shouldBe IdGenerationContext.DEFAULT_NAMESPACE.minor
      id.collection shouldBe collection
      EntityId.parse(id.value).toOption shouldBe Some(id)
    }
  }
}
