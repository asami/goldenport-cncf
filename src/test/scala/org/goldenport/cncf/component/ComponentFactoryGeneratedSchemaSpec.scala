...
final class ComponentFactoryGeneratedSchemaSpec extends AnyWordSpec with Matchers with GivenWhenThen {
  "ComponentFactory generated schema enrichment" should {
    ...
    "prefer explicit store record methods on reflective EntityPersistent bridges" in {
      ...
      And("the reflective bridge preserves the canonical storage identity without context repair")
      persistent
        .fromStoreRecord(persistent.toStoreRecord(entity))
        .map(
          _.asInstanceOf[_GeneratedStyleEntity].id.collection
        ) shouldBe Consequence.success(entity.id.collection)
    }
