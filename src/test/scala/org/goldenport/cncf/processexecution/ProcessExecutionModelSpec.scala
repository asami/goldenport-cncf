...
final class ProcessExecutionModelSpec extends AnyWordSpec with Matchers with GivenWhenThen {
  ...
  "Process Execution capability model" should {
    "identify capabilities and resolve runtime policy" which {
      ...
      "accept only safe WorkArea-relative paths and declared artifact metadata" in {
        ...
        val output =
          ProcessExecutionOutputDeclaration.createC(name, path, ProcessArtifactKind.File, 128L)
        val invalid =
          Vector("/tmp/result", "../result", "output/../../result", "output\\result", "")
      ...
      "resolve only a granted registered capability before any driver exists" in {
        ...
        val missing =
          policy.resolveC(request.copy(capability = unknown), ProcessExecutionGrant(unknown))
      ...
      "tighten runtime limits monotonically and reject widening overrides" in {
        ...
        val widening =
          ProcessExecutionLimits.effectiveC(maximum, ProcessExecutionLimits.empty, _limits(101L))
    ...
    "admit arguments and managed inputs" which {
      "apply the effective grant limit to input and declared artifacts before execution" in {
        ...
        val inputrejected =
          policy.resolveC(ProcessExecutionRequest(capability, input = input), grant)
        val outputrejected =
          policy.resolveC(ProcessExecutionRequest(capability, outputs = Vector(output)), grant)
      ...
      "admit only runtime-declared bounded input files before a driver is selected" in {
        ...
        val input =
          ProcessExecutionInputFile.createC(schema, path, Vector(1.toByte), 16L).toOption.get
        val rejectedinput =
          ProcessExecutionInputFile.createC(unknown, path, Vector(1.toByte), 16L).toOption.get
        ...
        val admitted =
          policy.resolveC(ProcessExecutionRequest(capability, inputFiles = Vector(input)), grant)
        val rejected = policy.resolveC(
          ProcessExecutionRequest(capability, inputFiles = Vector(rejectedinput)),
          grant
        )

        Then("only the registered logical input file is admitted")
        admitted.toOption.map(_.request.inputFiles.map(_.name.print)) shouldBe Some(
          Vector("schema")
        )
      ...
      "enforce declared argument vectors and input-file paths before a driver is selected" in {
        ...
        val input =
          ProcessExecutionInputFile.createC(schema, schemapath, Vector(1.toByte), 16L).toOption.get
        val alternate = ProcessExecutionInputFile.createC(
          schema,
          alternatepath,
          Vector(1.toByte),
          16L
      ...
      "reject an argument policy whose prefix differs from the runtime invocation" in {
        Given("a runtime definition whose argument policy declares a different fixed prefix")
        val capability = ProcessCapabilityId.parseC("agent-cli").toOption.get

        When("the trusted runtime constructs the program definition")
        ...
        Then("construction fails before component text can reach an unintended argument position")
        rejected.isFaillure shouldBe true
        rejected.toString should not include "--print"
        rejected.toString should not include "--execute"
      }

      "admit bounded dynamic text arguments only for an explicitly declared capability" in {
        Given("a shell-less capability that admits one bounded multiline text argument")
        val capability = ProcessCapabilityId.parseC("agent-cli").toOption.get
        val limits     = _limits(32L).copy(argumentCount = Some(1L), argumentBytes = Some(32L))
        val definition = ProcessProgramDefinition.fromRuntimeC(
          capability,
          "agent-cli",
          "/opt/cncf/bin/agent",
        ...
        When(
          "multiline, absent, blank, excess, oversized, and unsafe-control text arguments are submitted"
        )
        val admitted =
          policy.resolveC(ProcessExecutionRequest(capability, Vector("line one\nline two")), grant)
        val absent = policy.resolveC(ProcessExecutionRequest(capability), grant)
        val blank  = policy.resolveC(ProcessExecutionRequest(capability, Vector("  \n")), grant)
        val excess =
          policy.resolveC(ProcessExecutionRequest(capability, Vector("one", "two")), grant)
        val oversized =
          policy.resolveC(ProcessExecutionRequest(capability, Vector("x" * 33)), grant)
        val unsafe =
          policy.resolveC(ProcessExecutionRequest(capability, Vector("text\u001bescape")), grant)

        Then("only bounded safe text reaches the resolved shell-less execution")
        admitted.toOption.map(_.effectiveArguments) shouldBe Some(Vector(
          "--print",
          "line one\nline two"
        ))
        absent.isFaillure shouldBe true
        blank.isFaillure shouldBe true
        excess.isFaillure shouldBe true
        oversized.isFaillure shouldBe true
        unsafe.isFaillure shouldBe true
      }
    }

    "confine resource trees and WorkArea paths" which {
      "admit only program-declared opaque resource trees under narrowing limits" in {
        Given(
          "an admitted logical resource tree, a declared Process capability, and a smaller program cap"
        )
        val capability = ProcessCapabilityId.parseC("codex-cli").toOption.get
        ...
        val entry =
          ResourceTreeEntry.createC("schema.json", Vector(1.toByte, 2.toByte)).toOption.get
        val sourceLimits =
          ResourceTreeLimits(maxDepth = 2, maxEntries = 4, maxFileBytes = 8L, maxTotalBytes = 8L)
        val snapshot = ResourceTreeAccess.inMemory(Map(reference -> Vector(entry))).snapshot(
          reference,
          sourceLimits
        ).toOption.get
        val requested =
          ResourceTreeLimits(maxDepth = 1, maxEntries = 1, maxFileBytes = 2L, maxTotalBytes = 2L)
        val input =
          ProcessExecutionResourceTreeInput.createC(snapshot, target, requested).toOption.get
        val definition = _definition(capability, allowedresourcetrees = Map(reference -> requested))
        val policy     = ProcessExecutionPolicy.createC(Vector(definition)).toOption.get
        val grant =
          ProcessExecutionGrant(capability, resourceTreeLimits = Map(reference -> requested))

        When("the request supplies the declared tree and an undeclared tree identity")
        val admitted =
          policy.resolveC(ProcessExecutionRequest(capability, resourceTrees = Vector(input)), grant)
        val unknown = ResourceTreeReference.parseC("unknown").toOption.get
        val unknownsnapshot = ResourceTreeAccess.inMemory(Map(unknown -> Vector(entry))).snapshot(
          unknown,
          sourceLimits
        ).toOption.get
        val rejectedinput =
          ProcessExecutionResourceTreeInput.createC(unknownsnapshot, target).toOption.get
        val rejected = policy.resolveC(
          ProcessExecutionRequest(capability, resourceTrees = Vector(rejectedinput)),
          grant
        )

        Then(
          "admission retains the narrowed snapshot and rejects undeclared tree identities before a driver exists"
        )
      ...
      "leave tree limits unchanged when a capability grant has no tree-specific restriction" in {
        ...
        val limits =
          ResourceTreeLimits(maxDepth = 20, maxEntries = 2, maxFileBytes = 8L, maxTotalBytes = 8L)
      ...
      "reject resource-tree limit widening and WorkArea path collisions before filesystem access" in {
        Given(
          "an admitted tree with a bounded source policy and a fixed input path beneath its target"
        )
        val capability = ProcessCapabilityId.parseC("codex-cli").toOption.get
        val reference  = ResourceTreeReference.parseC("fixtures").toOption.get
        ...
        val limits =
          ResourceTreeLimits(maxDepth = 1, maxEntries = 1, maxFileBytes = 4L, maxTotalBytes = 4L)
        val snapshot = ResourceTreeAccess.inMemory(Map(reference -> Vector(entry))).snapshot(
          reference,
          limits
        ).toOption.get
        val widened =
          ProcessExecutionResourceTreeInput.createC(snapshot, target, ResourceTreeLimits.default)
        val schema    = ProcessArtifactName.parseC("schema").toOption.get
        val inputpath = WorkAreaRelativePath.parseC("input/schema.json").toOption.get
        val fixed =
          ProcessExecutionInputFile.createC(schema, inputpath, Vector(1.toByte), 4L).toOption.get
        ...
        Then(
          "the request is rejected without accepting broader limits or overlapping WorkArea intent"
        )
      ...
      "reject nested resource-tree targets before a WorkArea can be materialized" in {
        ...
        val firstsnapshot = ResourceTreeAccess.inMemory(Map(first -> Vector(entry))).snapshot(
          first,
          limits
        ).toOption.get
        val secondsnapshot = ResourceTreeAccess.inMemory(Map(second -> Vector(entry))).snapshot(
          second,
          limits
        ).toOption.get
        val firstinput = ProcessExecutionResourceTreeInput.createC(
          firstsnapshot,
          WorkAreaRelativePath.parseC("fixtures").toOption.get
        ).toOption.get
        val secondinput = ProcessExecutionResourceTreeInput.createC(
          secondsnapshot,
          WorkAreaRelativePath.parseC("fixtures/schema").toOption.get
        ).toOption.get
        val definition =
          _definition(capability, allowedresourcetrees = Map(first -> limits, second -> limits))
        val policy = ProcessExecutionPolicy.createC(Vector(definition)).toOption.get
      ...
      "reject input files whose WorkArea paths collide with declared outputs" in {
        ...
        val input =
          ProcessExecutionInputFile.createC(schema, inputpath, Vector(1.toByte), 16L).toOption.get
        val output = ProcessExecutionOutputDeclaration.createC(
          result,
          outputpath,
          ProcessArtifactKind.File,
        ...
        val definition =
          _definition(capability, Set(result), Set(schema), allowsworkingdirectory = true)
        val policy = ProcessExecutionPolicy.createC(Vector(definition)).toOption.get
    ...
    "preserve compatibility and runtime metadata boundaries" which {
