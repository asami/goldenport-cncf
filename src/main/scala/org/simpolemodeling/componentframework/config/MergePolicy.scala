...
import org.simplemodeling.componentframework.config.trace.{ConfigTrace, ConfigResolution}
import org.simplemodeling.componentframework.config.source.ConfigSource
...
object MergePolicy {
  ...
  def merge(
    ...
    source.load() match {
      case org.simplemodeling.Consequence.Success(cfg) =>
      ...
      case org.simplemodeling.Consequence.Failure(_) =>
        (current, trace)
    }
  }
