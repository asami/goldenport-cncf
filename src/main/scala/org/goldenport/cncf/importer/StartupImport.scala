...
object StartupImport {
  ...
  private def _resolve_paths(
    ...
    if (!Files.exists(path))
      Consequence.resourceNotFound(s"configured import path does not exist: ${path}")
    else if (Files.isRegularFile(path)) {
      if (!_is_supported_import_file(path))
        Consequence.resourceInvalid(s"unsupported import file extension: ${path}")
      else if (!Files.isReadable(path))
        Consequence.resourceInvalid(s"configured import file is not readable: ${path}")
      else
        Consequence.success(Vector(path.toAbsolutePath.normalize))
    } else if (Files.isDirectory(path)) {
      if (!Files.isReadable(path))
        Consequence.resourceInvalid(s"configured import directory is not readable: ${path}")
      else
        _collect_yaml_files(path)
    } else {
      Consequence.resourceInvalid(s"configured import path is not a file or directory: ${path}")
    }
  }
  ...
  private def _collect_yaml_files(
    ...
    try {
      ...
      case e: Throwable if scala.util.control.NonFatal(e) =>
        Consequence.resourceInvalid(s"failed to scan import directory ${dir}: ${e.getMessage}")
    }
  ...
  private def _read_text(
    ...
    try {
      ...
      case e: Throwable if scala.util.control.NonFatal(e) =>
        Consequence.resourceInvalid(s"failed to load import text ${path}: ${e.getMessage}")
    }
  ...
  private def _load_text_from_url(
    ...
    if (response.code != 200) {
      Consequence.resourceInvalid(s"startup import URL fetch failed: ${url} (HTTP ${response.code})")
    } else {
      response.getString match {
        case Some(body) =>
          Consequence.success(body)
        case None =>
          Consequence.resourceInvalid(s"startup import URL returned empty body: ${url}")
      }
    }
  }
  ...
  private def _parse_data_collection(
    ...
    if (normalized.isEmpty)
      Consequence.argumentMissing("collection")
  ...
  private def _parse_entity_collection(
    ...
    segments match {
      ...
      case _ =>
        Consequence.argumentInvalid(s"invalid entity collection id: ${value}")
    }
  }
