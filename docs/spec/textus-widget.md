...
Row-level issues are display metadata supplied by the row template or projected
row data. Editable collections should mark row issue containers with
`data-textus-row-issue` and `data-textus-issue-scope="row"` under the row that
already carries `data-textus-row`. WU-08 does not introduce a separate row
validation engine.
...
- `data-textus-validation-summary`: form-level validation summary.
- `data-textus-validation-message`: validation message scope or severity.
- `data-textus-validation-field`: canonical field name for a validation
  message.
- `data-textus-validation-code`: deterministic validation code.
- `data-textus-issue-scope`: `form`, `field`, or `row`.
- `data-textus-empty-state`: generated empty-state affordance.
...
WU-08 aligns validation, issue, capability, and empty-state feedback with this
selector contract. The renderer must preserve existing Bootstrap classes,
...
Disabled unauthorized controls keep their original control names and receive
`data-textus-capability-state="denied"` on the disabled affordance root.
...
When rendered, the message root carries
`data-textus-widget="textus:capability-message"`,
`data-textus-capability-message="denied"`,
`data-textus-capability-required`, and the policy used for the check.
...
- carries `data-textus-empty-state="true"` on generated empty-state roots.
...
- uses `data-textus-validation-summary="form"` for summaries and
  `data-textus-validation-message`, `data-textus-validation-field`,
  `data-textus-validation-code`, and `data-textus-issue-scope` for individual
  messages.
