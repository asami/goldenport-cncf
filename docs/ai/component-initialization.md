...
CNCF must discover components from search sources and active packaged sources and initialize them
with explicit runtime context. The process must be deterministic, explainable,
and compatible with non-classpath packaged sources.

## Why Naive Approaches Fail

- Classpath-only discovery ignores packaged source types and origins.
...
- ComponentRepository resolves search and active packaged source specs and produces discovered classes
...
- Debugging should identify which stage failed: packaged source discovery, factory
  selection, provider instantiation, or component initialization.
- Diagnostics should include packaged source spec, origin label, class loader, and
