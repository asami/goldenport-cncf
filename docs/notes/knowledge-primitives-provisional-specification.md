...
Phase 73 should inventory existing CNCF types before adding anything. Candidate reusable concepts are:
...
Where CaptureSession / CaptureItem are owned is intentionally not frozen by this note. Phase 73 should only pull them into CNCF if inventory confirms that they are domain-independent runtime primitives rather than KnowledgeHub acquisition-framework concepts.
...
Phase 61 established canonical Information runtime behavior. Phase 73 must begin with an inventory of that model and extend it compatibly.
...
Phase 73 is complete when a KnowledgeHub consumer can use CNCF types to represent and persist the generic result side of:
