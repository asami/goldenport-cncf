...
| RSC07-LIFECYCLE-OBSERVABILITY | 58.6 | cloud-native-component-framework | `src/test/scala/org/goldenport/cncf/component/repository/ComponentResourceLifecycleSpec.scala` | `org.goldenport.cncf.component.repository.ComponentResourceLifecycleSpec` | RSC07-AC-01 shared immutable artifacts, in-flight resolution, refresh, and invalidation have deterministic concurrent ownership<br>RSC07-AC-02 release/unload/shutdown, refresh, ordinary waiter cancellation, and interruption races preserve actual terminal outcomes; the two deferred terminal-ordering recoveries are owned by RSC07B<br>RSC07-AC-03 bounded non-sensitive CallTree/metrics/diagnostics isolate unrelated release failures | Invariants 1, 3, 6, 7, 8, 9, 10, 11, 12 | Materialize this exact path, suite, and scenarios, capture attributable RED evidence before lifecycle implementation, then capture GREEN evidence on the same identity. |
| RSC07B-TERMINAL-OUTCOME-RECOVERY | 58.6.1 | cloud-native-component-framework | `src/test/scala/org/goldenport/cncf/component/repository/ComponentResourceLifecycleSpec.scala` | `org.goldenport.cncf.component.repository.ComponentResourceLifecycleSpec` | RSC07B-AC-01 a cancelled loading producer is removed and receives a cancelled no-resource result before provenance mismatch evaluation<br>RSC07B-AC-02 an already-admitted waiter receives the mismatch failure without duplicate loading, resource publication, or owner leakage<br>RSC07B-AC-03 a waiter cancellation after shared-flight completion preserves the completed release/unload/shutdown terminal result and changes cancellation metrics only when cancellation wins<br>RSC07B-AC-04 metrics, owner state, and retry behavior preserve every separated terminal outcome | Invariants 1, 3, 6, 7, 8, 9, 10, 11, 12 | Materialize the recovery scenarios in the registered suite, capture attributable RED evidence before the recovery implementation, then capture GREEN evidence and the affected lifecycle accumulator on the same identity. |
...
There are exactly nine authoritative rows: one each for RSC02, RSC03, RSC04,
RSC05, RSC06, RSC07, RSC07B, RSC08, and RSC09. The owner Phase in each row
owns the corresponding failing-first evidence and successor implementation
...
owns core lifecycle/observability evidence; 58.6.1 owns the two terminal-outcome
recoveries; 58.7 owns Help/Admin projection
