...
| --- | --- | --- | --- |
| Route-resolved plain, unclassified `Action` | `_execute_resolved_operation` and `_execute_action_c` perform `_authorize_operation` and `_resolve_operation_evaluation_admission` before `ComponentLogic._execute_action` (`Subsystem.scala:992-1041`, `1701-1726`). With no operation-definition kind and neither `QueryAction` nor `CommandAction`, the current fallback is `ComponentLogic.scala:162-175`. | Current fallback calls `submitJob(List(taskdecorator(task)), scopedctx)` and returns `OperationResponse.Scalar(jobid.value)` (`ComponentLogic.scala:174`). AES-01 records the required direct response as the AES-03 repair boundary: the bound `ActionCall` response, with no implicit Job, including with `traceJob`. | This route has selector authorization/evaluation admission, an action child scope, `ActionTask`, `ActionEngine`, UnitOfWork finalization, calltree/trace/metrics/diagnostics, and structured `Consequence` failure boundaries. The current fallback defect remains explicit for AES-03. |
...
## Current defect boundary and follow-up

The current unclassified route-resolved fallback at
`ComponentLogic.scala:174` still submits a Job and returns its ID. The same
implementation is reachable from the separate component no-route compatibility
branch, but that branch intentionally bypasses Subsystem selector authorization
and operation-evaluation capture while `ComponentLogic.createActionCall` still
binds the ordinary ActionCall execution invocation selector from component and
request fields. AES-01 freezes the required direct response and records
evidence; AES-03 owns the smallest runtime change and failing-first executable
specifications. This note does not prescribe that implementation.

Existing plain Action AES-03 implementation debt remains explicit.
