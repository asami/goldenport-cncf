...
| Event/Job/runtime semantics | EventReception, EventStore fallback, transition lifecycle events, Job lifecycle publication, notification forwarding diagnostics, Job model defaults | route construction and mutation through runtime clock/ID controls; Event records, Job lifecycle publication, and CNCF-owned forwarding diagnostics are migrated |
