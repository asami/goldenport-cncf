...
Status: CV-01 implementation evidence; inventory only. This note does not
decide the CV-02 compatibility or ownership policy.

## Development-source admission

Phase 51 implementation changes are admitted only in development SNAPSHOT
source repositories. The owning repositories were transitioned before these
CV-01 docs/spec changes were reapplied:
...
No unavailable inter-repository SNAPSHOT was introduced. CNCF generation and
consumer compile inputs remain on the latest released prerequisites where a
local publication would be required: Cozy’s CNCF dependency remains `0.5.1`,
ArtScene’s CNCF compile dependency remains `0.5.1`, and consumer sbt-cozy
defaults remain released coordinates. No `publishLocal` or `publish` was run.

## Effective version-source inventory
...
### Omitted effective tracked sources

The following sources are part of the effective inventory and are recorded
here without normalizing their values:

| Tracked source | Exact current value | Ownership and lifecycle meaning | Contradiction or later mapping |
| --- | --- | --- | --- |
| Cozy `project/plugins.sbt` | `addSbtPlugin("org.goldenport" % "sbt-cozy" % "0.1.14")` | This is the sbt-cozy plugin used to build Cozy itself. It is a build-tool input for the Cozy repository, not the version of the Cozy CLI artifact and not the plugin version generated into a new CAR consumer. | It differs from the CozyScaffold generated default `0.1.15-SNAPSHOT` and from ArtScene's effective released `0.1.15` override. The ownership/precedence contract belongs to CV-06; development/release admission belongs to CV-07. |
| Cozy `src/main/scala/cozy/scaffold/CozyScaffold.scala` | `_default_sbt_cozy_version = "0.1.15-SNAPSHOT"` | This is the generated CAR scaffold's default `project/plugins.sbt` coordinate when no consumer override is supplied. It describes a newly generated consumer build, not the plugin used to build Cozy itself. | It differs from Cozy's own `0.1.14` and ArtScene's effective `0.1.15`. The distinction between scaffold default, consumer override, and Cozy build input maps to CV-06; mutable SNAPSHOT versus immutable release handling maps to CV-07. |
...
| ArtScene `src/main/car/assembly-descriptor.yaml` | subsystem and ArtScene component `0.1.1`; other components: AI runtime `0.2.1`, scraper `0.1.1`, toolchain runner `0.2.1` | This checked-in assembly descriptor identifies the packaged assembly/component set. It is a packaging/runtime input separate from the current `project.yaml` component version. | Its ArtScene `0.1.1` conflicts with `project.yaml` `0.1.2-SNAPSHOT`; it is not silently treated as a projection of the latter. Descriptor ownership, regeneration, and consistency map to CV-06. |
| ArtScene `src/main/car/component-descriptor.yaml` | component and descriptor version `0.1.1` | This checked-in component descriptor is packaging/runtime identity evidence for the CAR component. | It conflicts with the current project component version `0.1.2-SNAPSHOT` and is therefore evidence of a duplicated lifecycle source. Reconciliation maps to CV-06; no descriptor was modified in CV-01. |
...
| ArtScene `README.md` | component version `0.1.0-SNAPSHOT`; generated-source path Scala `3.3.7` | This is human-oriented generated-project guidance, not the authoritative build metadata. It records an older scaffold snapshot and older Scala path. | It conflicts with current `project.yaml` (`0.1.2-SNAPSHOT`, Scala `3.3.8`) and current generated/build assumptions. Documentation projection/ownership maps to CV-06; no README or build source was modified in CV-01. |
| ArtScene historical values | initial project `0.1.0-SNAPSHOT` (commit `478b557`), released `0.1.0` (commit `181c886`), released `0.1.1` (commit `73125f3`); runtime tested/minimum `0.4.13`, then `0.5.0`, then `0.5.1` | These values show the project/component and CNCF runtime compatibility lifecycle across historical releases. They are evidence, not current authority. | Historical release values explain why catalog/descriptors can remain `0.1.1` while the project advances to `0.1.2-SNAPSHOT`; they do not resolve the current duplication. Lifecycle and publication consistency map to CV-06/CV-07. |

## Lifecycle, contradictions, and gaps
...
## Exhaustive tracked-source scan
...
| Cozy `src/test/scala/cozy/archive/CozyArchivePackagerCv06Spec.scala` | whole-file scan passed | 9 of 9 Given/When/Then packaging-boundary behaviors cover required project directory, actual CAR classification, invalid classification, and no archive output | expanded archive/compatibility set: 51 passed, zero failed/canceled; `Test/compile` passed | pending |
| Cozy `src/test/scala/cozy/CozyArchivePackagerSpec.scala` | whole-file scan and current header passed | 30 of 30 archive behaviors have explicit Given/When/Then boundaries and five responsibility-level `which` sections | expanded archive/compatibility set: 51 passed, zero failed/canceled; `Test/compile` passed | pending |
...
| ArtScene `src/test/scala/org/simplemodeling/textus/artscene/impl/ArtSceneLauncherAssemblySpec.scala` | whole-file scan passed | Given/When/Then assembly behavior verifies packaged API isolation and excludes source-project implementation | ArtScene representative set: 5 passed, 1 intentional CV-06 cancellation, zero failed | pending |
...
3 scenarios, 0 pass, 3 canceled, 0 failed. `simplemodeling-model` is clean
and read-only.
