...
foundation=[Phase 57 Resource SubComponent Foundation](phase-57.md)
admin_consumer=[Phase 59 Component Admin and Documentation Visibility](phase-59.md)
...
Phase 58 organizes hand-written manuals, generated Help, Scaladoc, source,
...
Phase 57 is authoritative for Resource SubComponent identity, composition,
...
Phase 58 consumes that foundation and must not implement a second resolver.

Phase 58 does not reopen Phase 50 Entity revision/OCC behavior. It may document
...
- A Component knowledge package is projected over the logical release and
  resolved resource space supplied by Phase 57.
...
- Phase 59 Admin consumes the same Phase 57 resource view and Phase 58
  knowledge/model manifests; Phase 58 does not implement the Admin surface.
...
- Consume the Phase 57 composition manifest and resolved resource API without
...
- Bind Component knowledge and model resources to Phase 57 logical resource
  identities and physical provenance.
- Consume Phase 57 development, repository, offline, access, integrity, and
...
- Supply Phase 59 with the same knowledge/model manifest and exact resource
...
- Phase 58 does not make CAR embedding the primary public distribution of
  CNCF/CML/Cozy documentation.
- Phase 58 does not treat a framework Documentation Component as an
  independent source of truth; its publication generation and hashes must
  match the canonical source projection.
- Phase 58 does not make HTML scraping the preferred RAG ingestion path when
  structured SmartDox/publication projections are available.
- Phase 58 does not publish every `ai-directive` rule or raw `SKILL.md`;
...
- Phase 58 does not distribute Component documentation primarily through Maven
...
- Phase 58 does not reinterpret SubComponents as executable CAR dependencies
  or runtime Component participants.
- Phase 58 does not implement Component Admin runtime views or management
  actions; those belong to Phase 59.
...
| DOC-02 | Knowledge and model resource contracts | Versioned knowledge and Component-model manifests bind content, identity, provenance, disclosure, integrity, and codecs to the Phase 57 resource contract. | planned |
| DOC-03 | Authoring and content packaging toolchain | User Guide/Reference, model diagrams, Scaladoc, filtered release source, and Documentation/SourceCode content are generated through Cozy/SmartDox ownership for Phase 57 packaging. | planned |
| DOC-04 | Knowledge and development context composition | Knowledge, model, source, and framework publication resources compose over Phase 57 resolution into one attributed development context. | planned |
| DOC-05 | Unified Help and direct AI access | Help and AI consume one resolved Component resource space and publish the stable consumer contract later used by Phase 59 Admin. | planned |
...
- Phase 58 knowledge consumption uses the exact logical release, resolution,
  integrity, and availability results supplied by Phase 57.
- Help reports local, remote, restricted, unavailable, incompatible, stale,
  and corrupt states from Phase 57 without claiming physical content it did
  not resolve.
...
- Phase 59 can consume the same manifest/resource identities and navigation
...
- Phase 57 resolver-consumer and physical-provenance preservation
  specifications;
- Help/AI behavior for every Phase 57 availability and integrity state;
...
Phase 58 cannot close with behavior represented only in notes, journal, phase
documents, source code, or tests.

## Repository Responsibility

| Repository | Phase 58 responsibility |
| --- | --- |
| `/Users/asami/src/dev2025/cloud-native-component-framework` | Knowledge/model manifests, Phase 57 resolver consumption, Help/direct AI access, development context, authorization, and canonical CNCF contract |
| `/Users/asami/src/dev2025/cozy` and `sbt-cozy` | Authoring lint, model/diagram generation, Scaladoc/manual projection, filtered release-source capture, and content handoff to Phase 57 packaging |
...
After Phase 57 closes, begin DOC-01 with a cross-repository inventory. Freeze
...
changing Help routes. Treat Phase 57 composition/resolution as an input and
Phase 59 Admin as a later consumer.
