# Phase 94: Shared Transaction Domain for Workflow Suspension
...
checklist=[Phase 94 Checklist](phase-94-checklist.md)
...
Phase 94 is independent of Phases 89–93, including Phase 92 (Platform
Subcomponent CAR Distribution) and Phase 93 (Service Bus Runtime and Admin
Visibility). Its number does not make those Phases prerequisites.
It does not block Phase 77.1, Phase 77.2, or the first `sm-workflow`
vertical slice.

## Numbering note

This plan was renumbered from the local provisional Phase 92 label during
repository synchronization. GitHub's existing Phase 92 remains Platform
Subcomponent CAR Distribution; the renumbering does not change this plan's
scope or make intervening Phases prerequisites.
...
| ID | Outcome | Status |
| --- | --- | --- |
| WTX-94-01 | Freeze a versioned transaction-domain capability covering WorkflowInstance, Continuation, EventStore, DataStore, and the active UnitOfWork. | planned |
| WTX-94-02 | Implement one durable backend and opt-in UnitOfWork enlistment; reject default or mixed-store configurations before claimable work is published. | planned |
| WTX-94-03 | Prove one shared commit, prepare rejection, abort, commit failure/indeterminacy, restart recovery, and post-commit claim visibility. | planned |
| WTX-94-04 | Migrate the `sm-workflow` consumer only after compatibility and recovery evidence; preserve its existing loose boundary until then. | planned |
...
- The `AtomicWorkflowSuspensionEngine` and pure suspension-intent admission
  ...
  Phase 94 candidates only, outside Phase 77.1 and 77.2 closure.
