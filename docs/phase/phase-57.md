...
Phase 57 precedes Component Documentation and AI Knowledge Integration so
...
consumed by resolver policy and diagnostics. Phase 57 must not reopen Phase 55
...
- Documentation and SourceCode are initial closed roles. The model remains
  extension-ready, but arbitrary role registration is not part of Phase 57.
...
- Phase 58 Help and AI access and Phase 59 Component Admin consume the same
...
- Provide stable resolver/provenance APIs for Phase 58 Help/AI and Phase 59
  Admin consumers.
...
- Phase 57 does not author User Guides, Reference Manuals, Scaladoc, model
  diagrams, or AI indexes.
- Phase 57 does not implement Textus CBD Support or Textus BoK ingestion.
- Phase 57 does not implement final Help or Component Admin presentation.
- Phase 57 does not make a Resource SubComponent executable.
- Phase 57 does not turn a Resource SubComponent into a Component,
  Componentlet, Subsystem, service provider, or capability provider.
- Phase 57 does not add fine-grained language/media roles beyond the initial
  Documentation and SourceCode roles.
- Phase 57 does not use Maven documentation classifiers as the Component
  resource contract.
- Phase 57 does not allow a manifest to grant Operation authority, capability,
  source disclosure, or MCP readiness.
- Phase 57 does not treat ClassLoader loading as the resource-artifact
  lifecycle.
- Phase 57 does not package arbitrary `target` state. Source content policy and
  managed-source normalization are consumed by Phase 58.
