...
| CWF-77-10 | Freeze the minimum schema-versioned, fail-closed Continuation JSON wire contract and record exact CML-first cross-repository evidence; defer broad protocol extensions to later phases. | planned |

## Minimum Skill Continuation JSON Wire Contract

Phase 77 includes the smallest application-neutral wire contract needed for `sm-workflow` to hand a suspended operation to Skill and accept its result in a later process or turn. JSON is the standard wire encoding of this contract, not the canonical Workflow domain model.

- `ContinuationRequest` and `ContinuationResult` carry schema identity/version and use fail-closed JSON codecs.
- A request and result preserve Workflow and Continuation identity, expected revision, and `ContextSnapshot`.
- Required SPI typed input/result and Completion/Evidence requirements remain typed at the domain boundary; their JSON representation preserves the admitted schema identity/version.
- `WorkflowHandle` provides the minimum stable reference to the Workflow and its terminal or suspension state.
- A resume rejects unknown schema/version, incompatible typed payload, incorrect identity, stale revision/snapshot, missing required evidence, and duplicate/incompatible result according to the durable Continuation contract.

Phase 77 does not define a broad generic `Start` API, rich Presentation model, broad reasoning-level vocabulary, parent/child Workflow composition, orchestration, or REST/MCP/UI protocol surface. Those extensions are later-phase work.
...
CNCF projects an externally claimable Continuation SPI request into a compact Skill-facing command/WorkOrder. The projection is not the source of truth; Required SPI, the durable Continuation, and WorkflowInstance state are canonical.
...
- The minimum schema-versioned, fail-closed Continuation JSON wire contract is executable across a separate Skill process/turn without making JSON the canonical domain model.
- Broad generic Start APIs, rich Presentation, broad reasoning-level vocabularies, parent/child Workflow composition, orchestration, and REST/MCP/UI protocol surfaces are later-phase work.
...
- Broad generic Start API, rich Presentation, broad reasoning-level vocabulary, parent/child Workflow composition, orchestration, and REST/MCP/UI protocol extensions.
