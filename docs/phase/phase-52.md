# Phase 52 - Component Documentation and AI Knowledge Integration
...
Phase 52 organizes hand-written manuals, generated Help, Scaladoc, source,
...
Phase 52 does not reopen Phase 50 Entity revision/OCC behavior. It may document
...
- Phase 52 does not make CAR embedding the primary public distribution of
  CNCF/CML/Cozy documentation.
- Phase 52 does not treat a framework Documentation Component as an
  independent source of truth; its publication generation and hashes must
  match the canonical source projection.
- Phase 52 does not make HTML scraping the preferred RAG ingestion path when
  structured SmartDox/publication projections are available.
- Phase 52 does not publish every `ai-directive` rule or raw `SKILL.md`;
...
- Phase 52 does not distribute Component documentation primarily through Maven
...
- Phase 52 does not redesign the generic CAR executable dependency mechanism
...
Phase 52 cannot close with behavior represented only in notes, journal, phase
...
Phase 52 is planned and must not start before Phase 51 closes.
