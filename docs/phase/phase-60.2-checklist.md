...
- [ ] Show effective typed values, winning and overridden bindings, scope, source location, and selection trace.
- [ ] Consume the same Phase 58 already-resolved resource projection, `ResolvedComponentResources` or its accepted equivalent.
- [ ] Show primary, Documentation, and SourceCode artifact identity, availability, integrity, access, and physical provenance.
- [ ] Verify Admin performs no independent physical-resource scan or resolution and does not broaden the resource/mode policy.
