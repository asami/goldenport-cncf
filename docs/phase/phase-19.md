...
- The BI-04 projection/admin/manual slice adds `images` plus derived
  ...
  legacy `src/main/car/web`, Web application resources remain under
  `src/main/web`, and current Static Form Web descriptor source uses
  `src/main/web-inf` while non-public Web resources use `WEB-INF`.
...
- Blog read optimization uses CNCF View cache projections rather than making
  ...
  `BlogSlugIndex`, and author dashboards use `BlogAuthorPostView` with the
  standard CNCF `ViewCachePolicy` `Principal` query scope to avoid
  cross-author cache reuse.
